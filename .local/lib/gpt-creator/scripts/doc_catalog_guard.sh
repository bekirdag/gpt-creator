...
bootstrap_min_catalog() {
  ...
  if command -v gc_clone_python_tool >/dev/null 2>&1; then
    ...
    scripts_root="${GC_SCRIPTS_ROOT:-${helper_root}/scripts}"
    helper_path="${scripts_root}/python/bootstrap_min_catalog.py"
...
export GC_DOCS_MODE
if ! printf '{ "db": "%s", "enabled": %s, "mode": "%s" }\n' "$CAND" "$DOCS_ENABLED" "$GC_DOCS_MODE" > "$LOCK" 2>/dev/null; then
  printf >&2 "gpt-creator: warning: cannot write catalog lock at %s (continuing)\n" "$LOCK"
fi
