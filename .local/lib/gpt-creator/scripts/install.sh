...
as_root() {
  ...
  log_warn "Unable to elevate with sudo for ${target}; attempting without sudo."
  if "$@"; then
    return 0
  fi
  log_warn "Command failed without sudo: $* (target=${target}). Continuing."
  return 0
}
...
ensure_prefix_access() {
  ...
  for fallback in "${HOME}/.local" "${REPO_DIR}/.local"; do
    test_target="${fallback%/}/.gpt-creator-install-test.$$"
    if mkdir -p "$fallback" >/dev/null 2>&1 && touch "$test_target" >/dev/null 2>&1; then
      rm -f "$test_target" >/dev/null 2>&1 || true
      log_warn "No write access to ${INSTALL_PREFIX}; falling back to ${fallback}."
      INSTALL_PREFIX="$fallback"
      set_paths
      return 0
    fi
  done
  echo "✖ Cannot write to ${INSTALL_PREFIX}; rerun with --prefix \"${REPO_DIR}/.local\" or another writable prefix." >&2
  exit 1
}
...
install_completions() {
  ...
  if ! mkdir -p "$fdir" 2>/dev/null; then
    log_warn "Skipping fish completions (cannot write ${fdir})."
    return 0
  fi
  local fish_src="${REPO_DIR}/completions/gpt-creator.fish"
  if [[ -f "$fish_src" ]]; then
    install -m 0644 "$fish_src" "$fdir/gpt-creator.fish" 2>/dev/null || log_warn "Skipping fish completions (cannot install to ${fdir})."
  else
