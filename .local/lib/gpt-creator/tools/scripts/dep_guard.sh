...
if ! mkdir -p "$CACHE" "$STATE_DIR" 2>/dev/null; then
  warn "cannot write dependency state under $WORK_LOCK_DIR (skipping dep guard)"
  exit 0
fi
if ! touch "$STATE_DIR/.perm_check" 2>/dev/null; then
  warn "cannot write dependency state under $WORK_LOCK_DIR (skipping dep guard)"
  exit 0
fi
rm -f "$STATE_DIR/.perm_check" >/dev/null 2>&1 || true
