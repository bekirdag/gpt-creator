...
4. **Work Jira backlog** (optional):
  ...
  - `refine-tasks` reads the backlog from `tasks.db`, refines each task via Codex, writes the updates to `json/refined`, and persists the enriched fields back into SQLite immediately after each successful response.
...
- `refine-tasks` streams tasks from `tasks.db`, prompts Codex to enrich each one, writes the refined story JSON to `json/refined`, and synchronizes the updated fields back into SQLite immediately after each successful refinement.
