...
cmd_refresh_stack() {
  ...
  if [[ -f "$compose_file" && ${#services_to_start[@]} -gt 0 ]]; then
    local prune_helper="${CLI_ROOT}/scripts/python/prune_compose_services.py"
  ...
  if (( db_requested )); then
    ...
    if [[ -n "$db_container" ]]; then
      ...
      local wait_helper="${CLI_ROOT}/scripts/python/wait_for_mysql.py"
  ...
  if [[ -z "$db_container" ]]; then
    ...
    if (( skip_import == 0 || skip_seed == 0 )); then
      local db_helper="${CLI_ROOT}/scripts/python/refresh_stack_db.py"
      if [[ -f "$db_helper" ]]; then
        local db_output
