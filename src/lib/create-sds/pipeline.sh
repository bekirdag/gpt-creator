...
csds::assemble_document() {
  ...
  python3 - "$CSDS_JSON_DIR/manifest.json" "$CSDS_SECTIONS_DIR" "$final_path" "$CSDS_ASSEMBLY_DIR/table_of_contents.md" "$toc_json" <<'PY'
import json
import sys
from pathlib import Path
...
document_title = toc.get('document_title') or 'System Design Specification'
nodes = manifest.get('nodes') or []

lines = []
lines.append(f"# {document_title}")
lines.append('')
lines.append(f"_Generated by gpt-creator create-sds_\n")
lines.append(toc_md)
